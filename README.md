...
This repository contains a local Kaggriculture agent and the experiments used to improve it.

- `main.py` is the current submission agent. It exposes the required `agent(obs, config=None)` function.
- `debug_local.py` runs complete local seasons and prints daily state, market, and approximate P&L information.
- `baseline/history/` stores previous strategy versions for comparison and rollback.
- `baseline/` contains the original baseline notebooks and related reference material.
- `experiments/` contains benchmark helpers and experiment notes. Generated output is intentionally ignored by Git.
- `.vscode/launch.json` contains a ready-to-use VS Code debug configuration.

## Quick Start

The agent is CPU-friendly; a GPU is not required for the rule-based baseline or the local simulator.

```powershell
conda create -n kaggriculture python=3.11 -y
conda activate kaggriculture
pip install -U kaggle-environments -i https://pypi.tuna.tsinghua.edu.cn/simple --trusted-host pypi.tuna.tsinghua.edu.cn
```

Run a local season against the built-in `starter` agent:

```powershell
python debug_local.py --agent main.py --opponent starter --seed 17 --steps 720
```

For a faster smoke test, reduce `--steps`, for example `--steps 100`. The `EST_SELL_VALUE` and `EST_BUY_VALUE` lines are decision-support estimates based on the current market quote; they are not the official final reward.

### VS Code Debugging

Open this folder in VS Code, select **Debug Kaggriculture Agent** in the Run and Debug panel, and press `F5`. The default configuration runs 100 steps with seed 17 against `starter`. Breakpoints can be placed in `agent`, `_role_plan`, `_unit_actions`, or `_market_actions` in `main.py`.

### Kaggle Submission

Before submitting, join the competition and accept its rules on Kaggle. For a single-file submission:
...
Kaggle expects `main.py` at the submission root with an `agent` function. Keep imports self-contained because the submitted file runs in Kaggle's competition environment.

## Current Strategy Notes

The active agent uses a deterministic heuristic policy:

1. Assigns field roles based on available land and crop economics.
2. Plans movement and per-turn work for the farmer and hired hands.
3. Maintains crop and animal tasks such as planting, watering, feeding, harvesting, and pickup/drop-off.
4. Uses current market prices, demand signals, inventory, cash, and season timing to place market orders.

The historical strategy directories are useful for reproducible comparisons. The root `main.py` is the version to run and submit; experimental versions in `baseline/history/` are not automatically active.
