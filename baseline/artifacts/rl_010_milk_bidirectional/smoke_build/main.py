...
class RL010Policy:
    ...
    @staticmethod
    def _predict_model(model, features):
        if not model:
            return None
        ...
        uncertainty = max(1.0, rl010_float(model.get("uncertainty", 1.0)))
    ...
    def predict(self, event_key, action_name, features):
        ...
        if not model or rl010_int(model.get("support", 0)) < self.min_support:
            return None
        margin = self._predict_model(model.get("margin"), features)
        bad = self._predict_model(model.get("bad"), features)
...
class RL010Runtime:
    ...
    def act(self, obs, base_action):
        ...
        scored = []
        for action_name in RL010_ACTIONS:
            ...
            if safe and prediction["lcb"] > self.min_expected_delta and prediction["bad_ucb"] <= self.bad_ucb:
...
def rl010_fit_models(samples, ridge=8.0, min_support=RL010_MIN_SUPPORT):
    grouped = defaultdict(list)
    for row in samples:
        action_name = str(row.get("action", "")).upper()
        if action_name not in RL010_RATIOS:
            continue
        key = f"{rl010_opportunity_key(row)}|{action_name}"
        grouped[key].append(row)

    def fit_head(matrix, target):
        ...
        return {
            ...
            "uncertainty": max(1.0, float(np.std(residual)) * 1.25),
        }
    ...
    for key, rows in sorted(grouped.items()):
        ...
        model = {
            "support": support,
            "rows": len(rows),
            "margin": fit_head(matrix, target),
            "bad": fit_head(matrix, bad_target),
...
def _rl010_v27_agent(obs, config=None):
    try:
...
# RL-010: bidirectional MILK timing overlay.
RL010_PAYLOAD = {"version":"rl010","feature_dim":41,"min_support":24,"min_expected_delta":5.0,"lcb_z":1.5,"bad_ucb":0.1,"models":{}}
...
def agent(obs, config=None):
    try:
        base = _rl010_v27_agent(obs, config)
        return _RL010_RUNTIME.act(obs, base)
    except Exception:
        _RL010_RUNTIME.errors += 1
        return _rl010_v27_agent(obs, config)
