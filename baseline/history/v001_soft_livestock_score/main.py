...
def _livestock_score(obs, animal, own_count, opponent_count):
    ...
    wheat_price = float(
        prices.get("WHEAT", MARKET["WHEAT"][0]) or MARKET["WHEAT"][0]
    )
    day = int(obs.get("day", 0) or 0)
    days_left = max(0, TOTAL_DAYS - day)
    production_days = max(0, days_left - int(rule["first"]) + 1)
    cycles = (
        0
        if production_days <= 0
        else 1 + (production_days - 1) // int(rule["interval"])
    )

    # Compare marginal season economics instead of reacting to the current
    # product price alone. The sale haircut matches _market_actions, while
    # feed is paid every day and the animal purchase is paid immediately.
    expected_sales = cycles * price * 0.85
    expected_feed = days_left * wheat_price
    expected_net = expected_sales - expected_feed - float(rule["cost"])
    normalized_net = expected_net / float(max(1, rule["cost"]))
    demand_support = 1.0 + 0.012 * _town_demand_per_day(obs, product)
    crowding = 1.0 + 0.18 * opponent_count + 0.08 * own_count
    return (1.0 + normalized_net) * demand_support / crowding
