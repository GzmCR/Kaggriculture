...
# V011: high-value crop guard and pass-reducing overlay.
V011_HIGH_VALUE_CROPS = {"MELON", "STRAWBERRY", "TOMATO"}
V011_ENABLE_WATER_GUARD = True
V011_ENABLE_HARVEST_STORAGE = False
V011_STATS = {
    "last_step": -1,
    "calls": 0,
    "water_jobs_seen": 0,
    "harvest_jobs_seen": 0,
    "storage_jobs_seen": 0,
    "extra_water_assignments": 0,
    "extra_harvest_assignments": 0,
    "extra_drop_assignments": 0,
    "unassigned_urgent_jobs": 0,
    "pass_before": 0,
    "pass_after": 0,
    "unreachable_jobs": 0,
}
...
def _v011_reset(step):
    if step == 0 or step < int(V011_STATS.get("last_step", -1)):
        for key in (
            "calls",
            "water_jobs_seen",
            "harvest_jobs_seen",
            "storage_jobs_seen",
            "extra_water_assignments",
            "extra_harvest_assignments",
            "extra_drop_assignments",
            ...
            V011_STATS[key] = 0
    V011_STATS["last_step"] = step
    V011_STATS["calls"] += 1


def _v011_prices(obs, item):
    prices = ((obs.get("market", {}) or {}).get("prices", {}) or {})
    return float(prices.get(item, MARKET[item][0]) or MARKET[item][0])


def _v011_crop_jobs(obs, farm):
    day = int(obs.get("day", 0) or 0)
    hour = int(obs.get("hour", 0) or 0)
    step = int(obs.get("step", day * 24 + hour) or 0)
    jobs = []
    if not (V011_ENABLE_WATER_GUARD or V011_ENABLE_HARVEST_STORAGE):
        return jobs
    for y, row in enumerate(farm.get("tiles", []) or []):
        for x, tile in enumerate(row):
            if not isinstance(tile, dict) or tile.get("kind") != "PLANT":
                continue
            crop = tile.get("crop")
            if crop not in V011_HIGH_VALUE_CROPS or crop not in CROPS:
                continue
            rule = CROPS[crop]
            price = _v011_prices(obs, crop)
            age = day - int(tile.get("planted_day", day) or day)
            amount = int(tile.get("yield_units", 0) or 0)
            target = (x, y)
            if (
                V011_ENABLE_WATER_GUARD
                and not bool(tile.get("watered_today", False))
            ):
                critical = int(tile.get("consecutive_unwatered", 0) or 0) >= 1
                in_production_window = age >= max(0, int(rule["first"]) - 1)
                urgency = (
                    1_000_000
                    if critical
                    else 300_000
                    if in_production_window
                    else 100_000
                )
                expected_loss = price * max(1, int(rule["max_yield"]) - amount)
                jobs.append({
                    "kind": "WATER",
                    "target": target,
                    "action": ["WATER"],
                    ...
                    "deadline_step": step + (0 if critical else 23 - hour),
                    "urgent": critical or in_production_window,
                })
                V011_STATS["water_jobs_seen"] += 1

            if not V011_ENABLE_HARVEST_STORAGE or amount <= 0:
                continue
            if rule["ongoing"]:
                harvest_ready = (
                    amount >= int(rule["max_yield"]) - 1
                    or day >= 27
                )
            else:
                harvest_ready = (
            ...
            if harvest_ready:
                jobs.append({
                    ...
                    "action": ["HARVEST"],
                    "crop": crop,
                    "urgency": 850_000 if day >= 27 else 500_000,
                    "expected_loss": amount * price,
                    "deadline_step": step + (1 if day >= 27 else 23 - hour),
                    "urgent": True,
                })
                V011_STATS["harvest_jobs_seen"] += 1
    return jobs


def _v011_storage_jobs(obs, farm, private):
    if not V011_ENABLE_HARVEST_STORAGE:
        return []
    shed = private.get("shed", {}) or {}
    shed_load = sum(max(0, int(value or 0)) for value in shed.values())
    inventories = private.get("inventories", []) or []
    carried_load = sum(_inventory_total(inv) for inv in inventories)
    if shed_load + carried_load < 85:
        return []
    ...
    for index, inventory in enumerate(inventories):
        ...
        if product_load <= 0:
            continue
        target = _nearest_shed(positions[index], len(tiles), tiles)
        jobs.append({
            "kind": "DROP",
            "target": target,
            "action": ["DROP"],
            ...
            "crop": None,
            "urgency": 400_000,
            "expected_loss": 1000 * product_load,
            "deadline_step": int(obs.get("step", 0) or 0) + 1,
            "urgent": True,
        })
        V011_STATS["storage_jobs_seen"] += 1
    return jobs


def _v011_overlay(action, obs, config=None):
    farms = obs.get("farms", []) or []
    player = int(obs.get("player", 0) or 0)
    if not (0 <= player < len(farms)):
        return action
    ...
    private = obs.get("private", {}) or {}
    positions = [farm.get("farmer"), *(farm.get("hands", []) or [])]
    actions = [list(action.get("farmer", ["PASS"]))]
    actions.extend(list(item) for item in action.get("hands", []) or [])
    while len(actions) < len(positions):
        actions.append(["PASS"])
    V011_STATS["pass_before"] += sum(item == ["PASS"] for item in actions)

    jobs = _v011_crop_jobs(obs, farm)
    jobs.extend(_v011_storage_jobs(obs, farm, private))
    jobs.sort(
        key=lambda job: (
            -int(job["urgency"]),
            -float(job["expected_loss"]),
    ...
    for job in jobs:
        ...
        for index, position in enumerate(positions):
            if index in used_workers or position is None:
                continue
            if job.get("eligible") is not None and index != job["eligible"]:
                continue
            if actions[index] != ["PASS"]:
                continue
            target = tuple(job["target"])
            if job["kind"] != "DROP" and target in used_targets:
                continue
            distance = _distance(position, target)
            max_distance = (
                2
                if job["kind"] == "WATER" and job.get("urgent")
                else 0
                if job["kind"] == "WATER"
                else 2
            )
            if distance > max_distance:
                V011_STATS["unreachable_jobs"] += 1
                continue
            if distance == 0:
                next_action = list(job["action"])
            else:
                next_action = _bfs_first_step(
                    farm.get("tiles", []) or [], position, target
                )
            if not next_action or next_action == ["PASS"]:
                V011_STATS["unreachable_jobs"] += 1
                continue
            score = (
                int(job["urgency"])
            ...
            eligible.append((score, -distance, index, next_action))
        if not eligible:
            if job.get("urgent"):
                V011_STATS["unassigned_urgent_jobs"] += 1
            continue
        _, _, index, next_action = max(eligible)
        actions[index] = next_action
        used_workers.add(index)
        if job["kind"] != "DROP":
            used_targets.add(tuple(job["target"]))
        if job["kind"] == "WATER":
            V011_STATS["extra_water_assignments"] += 1
        elif job["kind"] == "HARVEST":
            V011_STATS["extra_harvest_assignments"] += 1
        elif job["kind"] == "DROP":
            V011_STATS["extra_drop_assignments"] += 1

    V011_STATS["pass_after"] += sum(item == ["PASS"] for item in actions)
    return {
        "farmer": actions[0] if actions else ["PASS"],
        "hands": actions[1:],
...
def agent(obs, config=None):
    ...
    _v011_reset(step)
    try:
        return _v011_overlay(_decide(obs, config), obs, config)
    except Exception:
        return _decide(obs, config)


# V011 refinement: feed the urgent crop jobs into the baseline allocator.
_V011_BASE_FIELD_JOBS = _field_jobs
_V011_BASE_RESET = _v011_reset
for _key in (
    "base_water_jobs",
    "base_harvest_jobs",
    "base_urgent_assignments",
):
    V011_STATS.setdefault(_key, 0)


def _v011_reset(step):
    previous = int(V011_STATS.get("last_step", -1))
    _V011_BASE_RESET(step)
...
def _v011_augmented_field_jobs(obs, farm, private, roles, liquidation):
    jobs = _V011_BASE_FIELD_JOBS(obs, farm, private, roles, liquidation)
    if liquidation:
        return jobs
    existing = {
        (tuple(job.get("target", ())), tuple(job.get("action", ())))
        for job in jobs
        if isinstance(job, dict)
    }
    for extra in _v011_crop_jobs(obs, farm):
        key = (tuple(extra["target"]), tuple(extra["action"]))
        if key in existing:
            continue
        if extra["kind"] == "WATER":
            critical = int(extra["urgency"]) >= 1_000_000
            if not critical:
                continue
            priority = 0
            reason = "v011_critical_water"
            V011_STATS["base_water_jobs"] += 1
        elif extra["kind"] == "HARVEST":
            priority = 2
            reason = "v011_high_value_harvest"
            V011_STATS["base_harvest_jobs"] += 1
        else:
            continue
        _add_job(
            jobs,
            priority,
            float(extra["expected_loss"]),
            extra["target"],
            extra["action"],
            reason=reason,
            latest_hour=23,
        ...
        jobs[-1].update({
            "urgency": int(extra["urgency"]),
            "deadline_step": int(extra["deadline_step"]),
            "expected_loss": float(extra["expected_loss"]),
            "crop_value": float(extra["expected_loss"]),
            "v011_urgent": bool(extra.get("urgent")),
        })
        existing.add(key)
    return jobs


_field_jobs = _v011_augmented_field_jobs
