...
ANIMAL_SLOTS = {"NW": 4, "NE": 5, "SW": 4, "SE": 0}
CROP_MIX = {
...
TRAVEL_COST = 8.0
FEED_STOCK_DAYS = 4
V006_ENABLE_FERTILIZER = True
V006_ENABLE_TERMINAL_FALLBACK = True
V006_TERMINAL_WORK_START = 704
V006_TREASURY_START = 710
V006_TREASURY_FLUSH = 718
LAND_OPEN_DAYS = (5, 9)
PRIORITY_BONUS = {
    -1: 120_000.0,
    0: 100_000.0,
...
def _fertilizer_opportunity(obs, tile, day):
    """Return (extra_units, net_value) when fertilizing beats selling it."""
    if not V006_ENABLE_FERTILIZER or not isinstance(tile, dict):
        return None
    crop = tile.get("crop")
    rule = CROPS.get(crop)
    fertilized_until = int(tile.get("fertilized_until_day", -1) or -1)
    if rule is None or fertilized_until > day:
        return None
    # Keep the first ablation deliberately conservative: fertilizer is an
    # opportunistic premium-crop action and must never displace today's water.
    if crop not in {"TOMATO", "STRAWBERRY", "MELON"}:
        return None
    ...
    prices = ((obs.get("market", {}) or {}).get("prices", {}) or {})
    product_price = float(
        prices.get(crop, MARKET[crop][0]) or MARKET[crop][0]
    )
    ...
    held = int(tile.get("yield_units", 0) or 0)
    if rule["ongoing"]:
        if age < rule["first"] or day >= 27:
            return None
        interval = int(rule["interval"])
        future_days = range(day + 1, min(TOTAL_DAYS, day + 4) + 1)
        production_days = sum(
            1
            for future_day in future_days
            if (future_day - int(tile.get("planted_day", day)) - rule["first"])
            % interval
            == 0
        )
        extra_units = min(max(0, rule["max_yield"] - held), production_days)
    else:
        in_window = (rule["max_day"] + 1) // 2 <= age <= rule["max_day"]
        if not in_window:
            return None
        remaining_days = rule["max_day"] - age + 1
        extra_units = min(
            max(0, rule["max_yield"] - held),
            2 * min(3, remaining_days),
        )
    gross_value = extra_units * product_price
    if extra_units <= 0 or gross_value <= 2.5 * fertilizer_price:
        return None
    return extra_units, gross_value - fertilizer_price
...
def _crop_jobs(obs, jobs, tile, target, day, liquidation):
    ...
    fertilizer = _fertilizer_opportunity(obs, tile, day)
    if fertilizer is not None:
        extra_units, net_value = fertilizer
        _add_job(
            jobs,
            5,
            net_value,
            ...
            ("FERTILIZE",),
            need="FERTILIZER",
            reason="fertilize_%s_%d" % (crop, extra_units),
            latest_hour=22,
        )
...
def _field_jobs(obs, farm, private, roles, liquidation):
    ...
    if not liquidation:
        ...
        if len(fertilizer_jobs) > 1:
            keep = max(
                fertilizer_jobs,
                key=lambda job: (float(job.get("value", 0.0)), job["target"]),
            )
            jobs = [
                job
                for job in jobs
                if job.get("need") != "FERTILIZER" or job is keep
            ]
    return jobs
...
def _terminal_local_action(farm, private, actor_index):
    """Use otherwise-idle workers for the last-turn local cleanup actions."""
    tiles = farm.get("tiles", []) or []
    positions = [farm.get("farmer"), *(farm.get("hands", []) or [])]
    if actor_index >= len(positions) or positions[actor_index] is None:
        return ["PASS"]
    x, y = (int(positions[actor_index][0]), int(positions[actor_index][1]))
    if not (0 <= y < len(tiles) and 0 <= x < len(tiles[y])):
        return ["PASS"]
    inventories = private.get("inventories", []) or []
    inventory = inventories[actor_index] if actor_index < len(inventories) else {}
    if (x, y) in _shed_tiles(len(tiles), tiles):
        if any(
            int(inventory.get(item, 0) or 0) > 0 for item in PRODUCTS
        ):
            return ["DROP"]
    tile = tiles[y][x]
...
def _apply_terminal_fallback(obs, farm, private, field):
    if not V006_ENABLE_TERMINAL_FALLBACK:
        return field
    step = int(obs.get("step", 0) or 0)
    if step < V006_TERMINAL_WORK_START:
        return field
    actions = [field.get("farmer", ["PASS"]), *field.get("hands", [])]
    for index, action in enumerate(actions):
        if action == ["PASS"]:
            actions[index] = _terminal_local_action(farm, private, index)
    return {
        "farmer": actions[0] if actions else ["PASS"],
        "hands": actions[1:],
        "liquidation": field.get("liquidation", True),
    }
...
def _unit_actions(obs, config, farm, private, roles):
    ...
    carried_fertilizer = sum(
        int(inv.get("FERTILIZER", 0) or 0) for inv in inventories
    )
    fertilizer_missing = max(0, len(fertilizer_jobs) - carried_fertilizer)
    fertilizer_pickups = min(
        len(positions), fertilizer_missing, shed_fertilizer, 2
    )
...
def _market_actions(obs, config, farm, private, roles, field):
    ...
    force_terminal_sales = (
        V006_ENABLE_TERMINAL_FALLBACK and step >= V006_TREASURY_START
    )
    ...
    for item in PRODUCTS:
        ...
        if item == "WHEAT" and left > 2 and not force_terminal_sales:
        ...
        quantity = (
            have
            if force_terminal_sales or step >= V006_TREASURY_FLUSH
            else _sell_quantity(item, have, inventory, day, shed_load, obs)
        )
        if quantity <= 0:
            continue
        proceeds = sum(
...
def _decide(obs, config=None):
    ...
    field = _apply_terminal_fallback(obs, farm, private, field)
    return {
        "farmer": field["farmer"],
        "hands": field["hands"],
