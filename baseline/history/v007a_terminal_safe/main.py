...
FEED_STOCK_DAYS = 3
LAND_OPEN_DAYS = (5, 9)
V007_ENABLE_TERMINAL = True
V007_ENABLE_IDLE_FERTILIZER = False
V007_TERMINAL_WORK_START = 704
V007_TREASURY_FLUSH = 718
V007_FERTILIZER_SAFETY_MULT = 3.0
PRIORITY_BONUS = {
    -1: 120_000.0,
    0: 100_000.0,
    1: 1_500.0,
...
def _v007_fertilizer_opportunity(obs, tile, day):
    """Estimate a deliberately conservative, local fertilizer opportunity."""
    if not V007_ENABLE_IDLE_FERTILIZER or not isinstance(tile, dict):
        return None
    crop = tile.get("crop")
    rule = CROPS.get(crop)
    ...
    if rule["ongoing"]:
        ...
        future_days = range(day + 1, min(TOTAL_DAYS - 1, day + 3) + 1)
        production_days = sum(
            1
        ...
        in_growth_window = (rule["max_day"] + 1) // 2 <= age < rule["max_day"]
        if not in_growth_window:
            return None
        remaining_days = max(0, rule["max_day"] - age)
        extra_units = min(
            max(0, rule["max_yield"] - held),
            2 * min(3, remaining_days),
        )
    gross_value = extra_units * product_price
    if extra_units <= 0 or gross_value < V007_FERTILIZER_SAFETY_MULT * fertilizer_price:
        return None
    return extra_units, gross_value - fertilizer_price


def _v007_has_fertilizer_opportunity(obs, farm, day):
    for row in farm.get("tiles", []) or []:
        for tile in row:
            if _v007_fertilizer_opportunity(obs, tile, day) is not None:
                return True
    return False


def _v007_apply_idle_fertilizer(obs, farm, private, field):
    """Overlay fertilizer only on units that baseline left idle."""
    if not V007_ENABLE_IDLE_FERTILIZER or field.get("liquidation"):
        return field
    ...
    for index, action in enumerate(actions):
        if action != ["PASS"] or index >= len(positions):
            continue
        ...
        if (
            ...
            and _v007_fertilizer_opportunity(obs, tile, day) is not None
        ):
            actions[index] = ["FERTILIZE"]
            continue
        if (
            not pickup_used
            and shed_fertilizer > 0
            and (x, y) in _shed_tiles(board_size, tiles)
            and int(inventory.get("FERTILIZER", 0) or 0) <= 0
            and _v007_has_fertilizer_opportunity(obs, farm, day)
        ):
            actions[index] = ["PICKUP", "FERTILIZER", 1]
            pickup_used = True
            shed_fertilizer -= 1
    return {
        "farmer": actions[0] if actions else ["PASS"],
        "hands": actions[1:],
        "liquidation": field.get("liquidation", False),
    }


def _v007_terminal_local_action(farm, private, actor_index):
    tiles = farm.get("tiles", []) or []
    positions = [farm.get("farmer"), *(farm.get("hands", []) or [])]
    ...
    if (x, y) in _shed_tiles(len(tiles), tiles):
        if any(int(inventory.get(item, 0) or 0) > 0 for item in PRODUCTS):
            return ["DROP"]
    return ["PASS"]


def _v007_apply_terminal_overlay(obs, farm, private, field):
    if not V007_ENABLE_TERMINAL:
        return field
    step = int(obs.get("step", 0) or 0)
    if step < V007_TERMINAL_WORK_START:
        return field
    actions = [field.get("farmer", ["PASS"]), *field.get("hands", [])]
    for index, action in enumerate(actions):
        if action == ["PASS"]:
            actions[index] = _v007_terminal_local_action(farm, private, index)
    return {
        "farmer": actions[0] if actions else ["PASS"],
        "hands": actions[1:],
...
def _v007_apply_overlays(obs, farm, private, field):
    field = _v007_apply_idle_fertilizer(obs, farm, private, field)
    return _v007_apply_terminal_overlay(obs, farm, private, field)
...
def _market_actions(obs, config, farm, private, roles, field):
    ...
    for item in PRODUCTS:
        ...
        quantity = (
            have
            if V007_ENABLE_TERMINAL and step >= V007_TREASURY_FLUSH
...
def _decide(obs, config=None):
    ...
    field = _v007_apply_overlays(obs, farm, private, field)
    return {
        "farmer": field["farmer"],
        "hands": field["hands"],
