...
def _strawberry_mix_shift(obs):
    """Reduce one premium-crop slot only during a visible local glut."""
    day = int(obs.get("day", 0) or 0)
    if day > CROPS["STRAWBERRY"]["last_plant"]:
        return 0
    prices = ((obs.get("market", {}) or {}).get("prices", {}) or {})
    price = float(
        prices.get("STRAWBERRY", MARKET["STRAWBERRY"][0])
        or MARKET["STRAWBERRY"][0]
    )
    opponent_supply = _opponent_visible_supply(
        obs, "STRAWBERRY", horizon=1
    )
    base = float(MARKET["STRAWBERRY"][0])
    if price <= 1.10 * base and opponent_supply >= 16:
        return -1
    return 0
...
def _role_plan(obs, farm):
    ...
    for quadrant in ("NW", "NE", "SW", "SE"):
        ...
        mix_shift = _strawberry_mix_shift(obs) if quadrant == "NE" else 0
        fixed["WHEAT"] = max(0, fixed.get("WHEAT", 0) - mix_shift)
        strawberry_count = max(
            0, len(cells) - sum(fixed.values()) + mix_shift
        )
        sequence = []
        if quadrant == "NW":
