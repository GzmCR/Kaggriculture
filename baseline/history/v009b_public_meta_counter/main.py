...
# V009B: public-meta detector plus a small market-only counter action.
V009B_PREMIUM = ("MELON", "STRAWBERRY", "MILK", "WOOL")
V009B_GLUT_WEIGHT = {"MELON": 3.6, "STRAWBERRY": 1.6, "MILK": 1.6, "WOOL": 3.2}
V009B_STATE = {
    "last_step": -1,
    "previous_inventory": {},
    "previous_prices": {},
...
def _v009b_reset(step):
    if step == 0 or step < int(V009B_STATE.get("last_step", -1)):
        V009B_STATE.update({
            "last_step": -1,
            "previous_inventory": {},
            "previous_prices": {},
...
def _v009b_market_snapshot(obs):
    market = obs.get("market", {}) or {}
    raw_inventory = market.get("inventory", {}) or {}
    raw_prices = market.get("prices", {}) or {}
    inventory = {
        item: int(raw_inventory.get(item, MARKET_I0) or MARKET_I0)
        for item in V009B_PREMIUM
    }
    prices = {
        item: float(raw_prices.get(item, MARKET[item][0]) or MARKET[item][0])
        for item in V009B_PREMIUM
    }
    old_inventory = V009B_STATE.get("previous_inventory", {})
    old_prices = V009B_STATE.get("previous_prices", {})
    deltas = {}
    regimes = {}
    for item in V009B_PREMIUM:
        inv_delta = inventory[item] - int(old_inventory.get(item, inventory[item]))
        price_delta = prices[item] - float(old_prices.get(item, prices[item]))
        base = float(MARKET[item][0])
        ratio = prices[item] / max(1.0, base)
        if ratio <= 0.90 or (inv_delta > 0 and price_delta < -1.0):
            regime = "GLUT"
        elif ratio >= 1.05 and (inv_delta <= 0 or price_delta >= 0.0):
            regime = "TIGHT"
        else:
            regime = "NORMAL"
        deltas[item] = (inv_delta, price_delta)
        regimes[item] = regime
    V009B_STATE["previous_inventory"] = inventory
    V009B_STATE["previous_prices"] = prices
    V009B_STATE["regimes"] = regimes
    return inventory, prices, deltas


def _v009b_public_counts(farm):
    counts = {
        "COW": 0,
        "SHEEP": 0,
        "GOOSE": 0,
...
def _v009b_public_meta_score(obs):
    step = int(obs.get("step", 0) or 0)
    day = int(obs.get("day", 0) or 0)
    if step % 24 != 0 or day < 6:
        return None
    farms = obs.get("farms", []) or []
    player = int(obs.get("player", 0) or 0)
    if not (0 <= player < len(farms)):
    ...
    counts = _v009b_public_counts(opponent)
    hands = len(opponent.get("hands", []) or [])
    unlocked = set(opponent.get("unlocked_quadrants", []) or [])
    score = 0
    # Some Kaggle observations mask the opponent's hands as an empty list.
    # Count this feature only when it is actually visible; never penalize a
    # candidate because a public field is masked.
    if hands and 10 <= hands <= 12:
        score += 1
    if unlocked == {"NW", "NE", "SW"}:
        score += 1
    if 7 <= counts["COW"] <= 9 and 4 <= counts["SHEEP"] <= 6 and counts["GOOSE"] == 0:
        score += 1
    # By day 12 the common tape has already harvested part of its melons and
    # wheat, so use production-stage evidence rather than exact crop counts.
    if day >= 10 and counts["MELON"] >= 5 and counts["STRAWBERRY"] >= 15:
        score += 1
    return score


def _v009b_update_profile(obs):
    score = _v009b_public_meta_score(obs)
    if score is None:
        return
    V009B_STATE["checkpoints"] += 1
    V009B_STATE["last_score"] = score
    if score >= 2:
        V009B_STATE["confidence"] = min(8, int(V009B_STATE["confidence"]) + 1)
        V009B_STATE["low_streak"] = 0
    else:
        V009B_STATE["confidence"] = max(0, int(V009B_STATE["confidence"]) - 1)
        V009B_STATE["low_streak"] = int(V009B_STATE["low_streak"]) + 1
    if not V009B_STATE["active"] and V009B_STATE["confidence"] >= 2:
        V009B_STATE["active"] = True
        V009B_STATE["activation_count"] += 1
    elif V009B_STATE["active"] and V009B_STATE["low_streak"] >= 2:
        V009B_STATE["active"] = False
        V009B_STATE["deactivation_count"] += 1


def _v009b_sell_score(order, prices, deltas, regimes):
    if not isinstance(order, list) or len(order) < 3 or order[0] != "SELL":
        return -1.0e30
    item = str(order[1])
    quantity = max(0, int(order[2] or 0))
    if item not in V009B_PREMIUM:
        return float(prices.get(item, MARKET.get(item, (1,))[0])) * quantity
    inv_delta, price_delta = deltas.get(item, (0, 0.0))
    regime_bonus = 1000.0 if regimes.get(item) == "TIGHT" else -1000.0 if regimes.get(item) == "GLUT" else 0.0
    return float(prices.get(item, MARKET[item][0])) * quantity * V009B_GLUT_WEIGHT[item] + regime_bonus + price_delta - inv_delta


def _v009b_overlay(action, obs, config=None):
    step = int(obs.get("step", 0) or 0)
    day = int(obs.get("day", 0) or 0)
    _v009b_reset(step)
    inventory, prices, deltas = _v009b_market_snapshot(obs)
    _v009b_update_profile(obs)
    V009B_STATE["last_step"] = step

    result = copy.deepcopy(action) if isinstance(action, dict) else {}
    market = result.get("market", [])
    if not isinstance(market, list):
        market = []
    market = [list(order) for order in market if isinstance(order, list) and order]
    max_orders = int((config or {}).get("maxMarketOrdersPerTurn", MAX_MARKET_ORDERS) or MAX_MARKET_ORDERS)
    market = market[:max_orders]
    if not V009B_STATE["active"] or not (8 <= day < 29):
    ...
    for order in market:
        if len(order) >= 3 and order[0] == "SELL":
            existing[str(order[1])] = existing.get(str(order[1]), 0) + max(0, int(order[2] or 0))

    choices = []
    for item in V009B_PREMIUM:
        regime = V009B_STATE["regimes"].get(item)
        inv_delta, price_delta = deltas.get(item, (0, 0.0))
        base = float(MARKET[item][0])
        price = prices.get(item, base)
        if regime == "GLUT" or price < 0.90 * base:
            continue
        if inventory.get(item, MARKET_I0) > 1.05 * MARKET_I0:
            continue
        if price_delta < -1.0 or inv_delta > 80:
            continue
        try:
            opponent_supply = _opponent_visible_supply(obs, item, horizon=1)
        except Exception:
            opponent_supply = 0
        if opponent_supply > 1.5 * max(1, _town_demand_per_day(obs, item)):
            continue
        available = max(0, int(shed.get(item, 0) or 0) - existing.get(item, 0))
        if available <= 0:
            continue
        score = price * V009B_GLUT_WEIGHT[item] + 2.0 * max(0.0, price_delta)
        choices.append((score, item, available))

    if choices:
        _, item, available = max(choices)
        front_quantity = min(available, max(1, min(4, int(math.ceil(available * 0.10)))))
        target_order = None
        for order in market:
        ...
        if target_order is not None:
            target_order[2] = int(target_order[2] or 0) + front_quantity
            V009B_STATE["modified_sells"] += front_quantity
        elif len(market) < max_orders:
            market.append(["SELL", item, front_quantity])
            V009B_STATE["extra_sells"] += front_quantity
        ...
        sorted_sells = sorted(
            sells,
            key=lambda order: _v009b_sell_score(order, prices, deltas, V009B_STATE["regimes"]),
            reverse=True,
        )
        if sells != sorted_sells:
            V009B_STATE["reordered_sells"] += 1
            for index, order in zip(sell_indices, sorted_sells):
                market[index] = order
...
def agent(obs, config=None):
    try:
        baseline_action = _decide(obs, config)
        return _v009b_overlay(baseline_action, obs, config)
    except Exception:
        return _decide(obs, config)
