...
# V009A: market-memory overlay.  The field planner above is intentionally
# untouched; only the market list returned by the baseline is adjusted.
V009A_PREMIUM = ("MELON", "STRAWBERRY", "MILK", "WOOL")
V009A_GLUT_WEIGHT = {"MELON": 3.6, "STRAWBERRY": 1.6, "MILK": 1.6, "WOOL": 3.2}
V009A_STATE = {
    "last_step": -1,
    "previous_inventory": {},
    "previous_prices": {},
...
def _v009a_reset(step):
    if step == 0 or step < int(V009A_STATE.get("last_step", -1)):
        V009A_STATE.update({
            "last_step": -1,
            "previous_inventory": {},
            "previous_prices": {},
...
def _v009a_snapshot(obs):
    ...
    inventory = {
        ...
        for item in V009A_PREMIUM
    }
    prices = {
        item: float(raw_prices.get(item, MARKET[item][0]) or MARKET[item][0])
        for item in V009A_PREMIUM
    }
    previous_inventory = V009A_STATE.get("previous_inventory", {})
    previous_prices = V009A_STATE.get("previous_prices", {})
    regimes = {}
    deltas = {}
    for item in V009A_PREMIUM:
        base = float(MARKET[item][0])
        inv_delta = inventory[item] - int(previous_inventory.get(item, inventory[item]))
        price_delta = prices[item] - float(previous_prices.get(item, prices[item]))
    ...
    V009A_STATE["previous_inventory"] = inventory
    V009A_STATE["previous_prices"] = prices
    V009A_STATE["regimes"] = regimes
    return inventory, prices, deltas


def _v009a_order_score(order, prices, deltas, regimes):
    if not isinstance(order, list) or len(order) < 3 or order[0] != "SELL":
        return -1.0e30
    item = str(order[1])
    quantity = max(0, int(order[2] or 0))
    if item not in V009A_PREMIUM:
        return float(prices.get(item, MARKET.get(item, (1,))[0])) * quantity
    inv_delta, price_delta = deltas.get(item, (0, 0.0))
    trend = 0.0
    if regimes.get(item) == "TIGHT":
        trend += 0.25 * float(MARKET[item][0])
    elif regimes.get(item) == "GLUT":
        trend -= 0.25 * float(MARKET[item][0])
    trend += 2.0 * max(0.0, price_delta) - 0.5 * max(0, inv_delta)
    return float(prices.get(item, MARKET[item][0])) * quantity * V009A_GLUT_WEIGHT[item] + trend


def _v009a_overlay(action, obs, config=None):
    step = int(obs.get("step", 0) or 0)
    day = int(obs.get("day", 0) or 0)
    _v009a_reset(step)
    inventory, prices, deltas = _v009a_snapshot(obs)
    V009A_STATE["last_step"] = step
    ...
    # In a glut, trim only the risky premium component.  Keep one unit so the
    # baseline still realizes cash and retain all non-premium orders verbatim.
    adjusted = []
    for order in market:
        if (
            day < 29
            and len(order) >= 3
            and order[0] == "SELL"
            and order[1] in V009A_PREMIUM
            and V009A_STATE["regimes"].get(order[1]) == "GLUT"
        ):
            quantity = max(0, int(order[2] or 0))
            reduced = max(1, int(quantity * 0.75)) if quantity > 1 else quantity
            if reduced < quantity:
                V009A_STATE["reduced_sells"] += quantity - reduced
                order[2] = reduced
        adjusted.append(order)
    market = adjusted

    # Sort only existing premium SELL slots.  This preserves the baseline's
    # buy/seed/land order while letting tight items use earlier market slots.
    sell_indices = [
        index for index, order in enumerate(market)
        if len(order) >= 3 and order[0] == "SELL"
    ]
    sell_orders = [market[index] for index in sell_indices]
    sorted_sells = sorted(
        sell_orders,
        key=lambda order: _v009a_order_score(order, prices, deltas, V009A_STATE["regimes"]),
        reverse=True,
    )
    if sell_orders != sorted_sells:
        V009A_STATE["reordered_sells"] += 1
        for index, order in zip(sell_indices, sorted_sells):
            market[index] = order

    # Add at most one modest premium sale in a genuine tight/recovering market.
    # Never use this during terminal liquidation, and never touch feed wheat.
    if 8 <= day < 29 and len(market) < max_orders:
        private = obs.get("private", {}) or {}
        shed = private.get("shed", {}) or {}
        existing = {
            str(order[1]): max(0, int(order[2] or 0))
            for order in market
            if len(order) >= 3 and order[0] == "SELL"
        }
        choices = []
        for item in V009A_PREMIUM:
            if V009A_STATE["regimes"].get(item) != "TIGHT":
                continue
            inv_delta, price_delta = deltas.get(item, (0, 0.0))
            base = float(MARKET[item][0])
            current_price = prices.get(item, base)
            market_stock = inventory.get(item, MARKET_I0)
            if current_price < 0.95 * base or market_stock > 1.03 * MARKET_I0:
                continue
            ...
            if available <= 0:
                continue
            score = current_price * V009A_GLUT_WEIGHT[item] + 2.0 * max(0.0, price_delta)
            choices.append((score, item, available))
        if choices:
            _, item, available = max(choices)
            quantity = min(available, max(1, min(4, int(math.ceil(available * 0.10)))))
            market.append(["SELL", item, quantity])
            V009A_STATE["extra_sells"] += quantity
...
def agent(obs, config=None):
    try:
        ...
        return _v009a_overlay(baseline_action, obs, config)
    except Exception:
        return _decide(obs, config)
