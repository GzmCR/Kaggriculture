"""Evaluate the V007 promotion gate from the merged benchmark CSV."""
...
def main():
    with (ARTIFACTS / "v007_results.csv").open(newline="") as handle:
        rows = list(csv.DictReader(handle))
    current = [row for row in rows if row["candidate"] == "current"]
    if not current:
        raise SystemExit("missing current control rows")
    ...
    for candidate in MAIN:
        ...
        hamburger = [
            row for row in group if row["opponent"] == "hamburger"
        ]
        metrics = {
    ...
    (ARTIFACTS / "v007_gate.json").write_text(json.dumps(report, indent=2))
